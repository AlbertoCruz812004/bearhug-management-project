...
export const RenderTable = ({ useAsyncList, isLoading, columns, renderCell }: Props) => {
  const renderCustomCell = useCallback((item: typeof useAsyncList.items, columnKey: Key) => {
  ...
  return (
    <Table isStriped selectionMode="single" color="primary" sortDescriptor={useAsyncList.sortDescriptor}
      ...
      >
        {(item) => (
          <TableRow key={item.barcode}>
            {(columnKey) => <TableCell>{renderCell ? renderCustomCell(item, columnKey) : getKeyValue(item, columnKey)}</TableCell>}
          </TableRow>
        )}
      </TableBody>
    </Table>
